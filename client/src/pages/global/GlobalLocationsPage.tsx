...
// Column definitions for the global locations table
const columns: ColumnDef<GlobalLocation>[] = [
  {
    accessorKey: "name",
    header: ({ column }) => (
...
export function GlobalLocationsPage() {
  ...
  const { data, isLoading, refetch, isRefetching } = useGlobalLocations(pagination);

  const handleRowClick = (location: GlobalLocation) => {
    navigate(`/org/${location.organization_id}/locations/${location.id}`);
  };
    ...
    <Card>
      <CardContent className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-sm text-muted-foreground text-center mb-4">
          No locations have been created across any organization
        </p>
      </CardContent>
    </Card>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight">All Locations</h1>
          <p className="text-muted-foreground mt-1">
            View locations across all organizations
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        showColumnToggle
        columnVisibilityStorageKey="column-visibility-global-locations"
        onRefresh={() => { refetch(); }}
